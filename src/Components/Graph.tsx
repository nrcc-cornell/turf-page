...
export default function Graph(props: GraphData & { units: string }) {
  ...
  const getForecastData = () => {
    ...
    const todayIdx = props.table[0].findIndex(
      (arr: [string, number]) => arr[0] === todayStr
    );

    let forecastData = props.table[0]
      .slice(todayIdx + 1)
      .map((arr): [string, number] => [arr[0].slice(0, 5), arr[1]]);
  ...
  const options = {
    ...
    title: {
      text: `Current, Last, and Normal Season ${
        props.units === 'GDDs' ? 'GDD' : 'Precipitation'
      } Accumulation`,
    },
    series: [
      {
      ...
      },
      {
        ...
        name:
          props.current.length > 0 ? props.current[0][0].slice(6) : 'Current',
        color: 'rgb(163,41,41)',
        zIndex: 3,
        marker: {
          symbol: 'circle',
          ...
          return (
            <Fragment key={p.series.name}>
              ...
              <Box style={{ justifySelf: 'right' }}>
                <span style={{ fontWeight: 'bold' }}>{p.y}</span> {props.units}
              </Box>
            </Fragment>
          );
        });
