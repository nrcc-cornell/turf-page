...
export default function RiskGraph(props: RiskGraph) {
  ...
  const datesConverted = props.data['season'].map((arr: [string, number]) => [arr[0].slice(0,5), Math.max(0, arr[1])]);
  ...
  if ('7 Day Avg' in props.data) {
    const otherDatesConverted = props.data['7 Day Avg'].map((arr: [string, number]) => [arr[0].slice(0,5), Math.max(0, arr[1])]);
    ...
    tooltip: {
      ...
      formatter: function(this: Highcharts.TooltipFormatterContextObject) {
        ...
        const dataElems = this.points.map(p => {
          ...
          return (
            <Fragment key={p.series.userOptions.id}>
              ...
              <Box style={{ justifySelf: 'right', fontWeight: 'bold' }}>{typeof p.y === 'number' ? roundXDigits(p.y, 2) : 'null'}</Box>
            </Fragment>
          );
        });
        ...
        return renderToStaticMarkup(<Box style={{
          ...
          <Box style={{ fontSize: '12px', fontStyle: 'italic', textAlign: 'center' }}>({this.points[0].series.userOptions.id?.split(' ')[0]})</Box>
